...
router.get('/GetAllAnnouncements',(req,res) => {
    announcements.find().sort({createdAt:-1})
    .then((result)=>{
    	res.send(result)
...
router.get('/DeleteAnnouncements/:id',(req,res)=>{
	announcements.findByIdAndDelete(req.params.id)
	.then((result)=>{
    	res.send(result)
    	//res.render('page_name',{issues:result})
    })
...
router.get('/CommentedAnnouncement',(req,res) => {
    announcements.find({'Comments.userID':2})
    .then((result)=>{
    	res.send(result)
    	//res.render('page_name',{issues:result})
    })
...
router.get('/FilteredAnnouncement',(req,res) => {
    ...
    prog=['','Btech']
    dept=['','CSE']
    announcements.find({'Filter.Batch':{$in:batch},'Filter.Dept':{$in:dept},'Filter.ProgType':{$in:prog}}).sort({createdAt:-1})
    .then((result)=>{
        res.send(result)
        //res.render('page_name',{issues:result})
    })
...
router.get('/CommentedAnnouncement/:id',(req,res)=>{
	var c="Agreed yes."
	var cid=8
	announcements.findByIdAndUpdate(req.params.id,{ $push: {'Comments.comment':c ,'Comments.userID':cid }})
	.then((result)=>{
    	res.send(result)
    	//res.render('page_name',{issues:result})
    })
