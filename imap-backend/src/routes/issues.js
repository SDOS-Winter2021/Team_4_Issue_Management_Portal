...
router.get('/addIssue',(req,res) => {
    ...
    const issue=new issues({
        ...
        Desc: 'Please reduce number of quizzes and everything man at this point mai sirf bakwas likh rahi hoon kyunki I have run out of issues or maybe I am too lazy to think of issues cuz no one can ever RUN out of issues, am I right? lol',
...
router.get('/CommentedIssue/:id',(req,res)=>{
	var c="Agreed yes."
	var cid=8
	issues.findByIdAndUpdate(req.params.id,{ $push: {'Comments.comment':c ,'Comments.userEmail':c }})
	.then((result)=>{
    	res.send(result)
    	//res.render('page_name',{issues:result})
    })
